...
def construct_pyg_data(adata, val_mask, test_mask):
    ...
    # 动态调整主成分数，确保不超过特征数
    max_pca_components = min(50, adata.shape[1])  # 根据特征数调整主成分数
    gene_graph_pca_num = max_pca_components  # 设置最终的主成分数
...
for data_mode in data_modes:
    for sample_number in benchmark_samples:
            ...
            for split_version in range(10):
                ...
                h5ad_path = os.path.join(data_dir, "osmfish_20240902194203.h5ad")
                adata = load_and_process_h5ad(h5ad_path)

                # 设置路径
                split_dir = r"./OSMFISH/DataSplit/all_gene"
...
results_df.to_csv("OSMFISH_results.csv", index=False)
